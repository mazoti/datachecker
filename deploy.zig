...
pub fn main(init: std.process.Init) !void {
    ...
    const current_directory: []u8      = try std.process.currentPathAlloc(init.io, arena);
    ...
    var input_dir: std.Io.Dir        = try std.Io.Dir.openDirAbsolute(init.io, input_folder, .{ .iterate = true });
    ...
    while (try walker.next(init.io)) |entry| {
        ...
        if (entry.kind == .file) {
            ...
            const input_bin: []const u8    = try std.fmt.bufPrint(&buffer, "zig-out{c}{s}", .{ std.fs.path.sep, entry.path });

            var child: std.process.Child   = try std.process.spawn(init.io, .{ .argv = &[_][]const u8{"xz", "-z", "-9", "-e", input_bin }});
            ...
            const old_dir: std.Io.Dir      = try std.Io.Dir.openDirAbsolute(init.io, old_dir_tmp, .{ .iterate = true });
            ...
            const new_dir: std.Io.Dir      = try std.Io.Dir.openDirAbsolute(init.io, new_dir_tmp, .{ .iterate = true });
            ...
            // Creates empty file to hash later
